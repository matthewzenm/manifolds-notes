...
\begin{proof}
    ...
    \begin{align*}
        f(u)-f(0)=&\int_0^1\frac{\mathrm{d}f(tu)}{\mathrm{d}{t}}\ \mathrm{d}{t}\\
...
\begin{defn}
    设$M$是$\tilde{M}$的子流形, $\tilde{M}$中一个\textbf{沿$M$的向量场}为一个光滑映射$X:M\to T\tilde{M}$, 满足$\pi\circ X=1_M$.
    $\tilde{M}$中所有沿$M$的向量场的集合记为$\Gamma(T\tilde{M}|_M)$\footnote{这个记号来自于向量丛的截面.}.
\end{defn}
\begin{rem}
    ``沿$M$的向量场''和``$M$上的向量场''是有区别的.
    对前者而言, 每点$p$处的切向量可以在$T_p\tilde{M}$中取值, 而后者的切向量只能在$T_pM$中取值.
...
作为定理~\ref{orientable iff normal field}~的结束, 我们来讨论在第~\ref{sect_of_mani}~节结尾处留下的问题, 即证明M\"{o}bius带是不可定向的.
\begin{eg}
...
\subsection*{$\mathfrak{X}(M)$的自同态}
本笔记中需要用到这一小节的地方离我们还有点远, 第一次阅读时可以先跳过这一小节.